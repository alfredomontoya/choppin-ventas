...
export default function Index({ clientes, filtros }: { clientes: any; filtros: any }) {
  ...
  useEffect(() => {
    if (filtros.por_pagina === undefined) {
      ...
      if (pp !== 10) {
        router.get(route('clientes.index', { ...filtros, por_pagina: pp, busqueda: search || undefined }), {}, { replace: true });
      }
    }
  }, []);
  ...
  const limpiarFiltros = () => {
    setSearch('');
    router.get(route('clientes.index'), {}, { preserveState: true, replace: true });
  };
  ...
  const cambiarPagina = (params: Record<string, any>) => {
    router.get(route('clientes.index'), { ...filtros, ...params, busqueda: search }, { preserveState: true, replace: true });
  };
  ...
  const eliminar = () => {
    if (!deleteId) return;
    router.delete(route('clientes.destroy', deleteId), {
      onSuccess: () => setDeleteId(null),
    });
  };

  const resultados = clientes.total ?? 0;
      ...
      <div className="space-y-6">
        ...
        <Card>
          <div className="flex flex-wrap gap-3 mb-4">
            <div className="relative flex-1 min-w-[200px] max-w-md">
              ...
              {search && (
                <button
                  onClick={() => {
                    setSearch('');
                    router.get(route('clientes.index'), { ...filtros, busqueda: undefined }, { preserveState: true });
                  }}
            ...
            <button
              onClick={() => { window.location.href = route('clientes.exportar', filtros); }}
